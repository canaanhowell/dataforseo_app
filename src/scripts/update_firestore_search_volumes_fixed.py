...
Fixed version that properly handles keywords with special characters by wrapping them in quotes.

This script:
1. Fetches keywords from Firestore
2. Gets monthly search volume data from DataForSEO API
3. Outputs JSON with monthly breakdown
4. Optionally updates Firestore with the new data
"""
...
class FirestoreKeywordUpdater:
    ...
    def clean_keyword_for_api(self, keyword: str) -> str:
        """
        Clean keyword for DataForSEO API by wrapping in quotes if it contains special characters.
        
        Args:
            keyword: Original keyword
            
        Returns:
            Cleaned keyword (wrapped in quotes if needed)
        """
        # Check if keyword contains special characters that need quoting
        special_chars = [',', '.', '!', '?', ':', ';', '(', ')', '[', ']', '{', '}']
        
        if any(char in keyword for char in special_chars):
            # Wrap in double quotes for exact match search
            cleaned = f'"{keyword}"'
            logger.info(f"Wrapped keyword in quotes: {keyword} -> {cleaned}")
            return cleaned
    ...
    async def get_monthly_search_volumes(
        ...
        for keyword in keywords:
            cleaned = self.clean_keyword_for_api(keyword)
            ...
            for i in range(0, len(cleaned_keywords), batch_size):
                ...
                try:
                    ...
                    for result in search_results:
                        ...
                        # If the result keyword is quoted, try to find the original
                        if result.keyword.startswith('"') and result.keyword.endswith('"'):
                            unquoted = result.keyword[1:-1]
                            if unquoted in keywords:
                                original_keyword = unquoted
...
async def main():
    ...
    try:
        ...
        # Count keywords that will be quoted
        keywords_to_quote = [k for k in keywords if any(char in k for char in [',', '.', '!', '?', ':', ';', '(', ')', '[', ']', '{', '}'])]
        if keywords_to_quote:
            logger.info(f"Keywords that will be quoted for API: {len(keywords_to_quote)}")
            for k in keywords_to_quote[:5]:  # Show first 5
                logger.info(f"  - {k}")
            if len(keywords_to_quote) > 5:
                logger.info(f"  ... and {len(keywords_to_quote) - 5} more")
